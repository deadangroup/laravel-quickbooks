...
class QBOClient
{
     ...
     *
     * @param  array  $configs
     * @param  QBOToken  $token
     */
    public function __construct(array $configs, QBOToken $token)
    {
     ...
     * @return $this
     */
    public function setToken(QBOToken $token)
    {
        $this->token = $token;
    ...
    protected function makeDataService()
    {
        ...
        if ($this->hasValidRefreshToken()) {
            // Pull in the configs from the token into needed keys from the configs
            $data_service = DataService::Configure(array_merge(array_intersect_key($this->parseDataConfigs(),
                $existing_keys), [
                'QBORealmID'      => $this->token->realm_id,
                'refreshTokenKey' => $this->token->refresh_token,
            ]));
    ...
    public function exchangeCodeForToken($code, $realm_id)
    {
        ...
        $company = $this->getDataService()
                        ->getCompanyInfo();

        $this->token->parseCompanyDetails($company)
                    ->save();
